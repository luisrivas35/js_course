...
const fruts = [
  { id: "sandia", icon: "🍉", price: 3 },
  { id: "pera", icon: "🍐", price: 5 },
  { id: "manzana", icon: "🍎", price: 1 },
];

document.addEventListener("DOMContentLoaded", () => {
  const addFrut = document.querySelector("#addFrut");
  const deleteFirstFrut = document.querySelector("#deleteFirstFrut");
  const deleteLastFrut = document.querySelector("#deleteLastFrut");
  const listFruts = document.querySelector("#listFruts");
  const priceASC = document.querySelector("#priceASC");
  const priceDESC = document.querySelector("#priceDESC");
  const orderByName = document.querySelector("#orderByName");

  const renderFruts = () => {
    listFruts.innerHTML = "";
    fruts.forEach((frut) => {
      listFruts.innerHTML += `<li>${frut.id} - ${frut.icon} - ${frut.price}</li>`;
    });
  };
  renderFruts();

  // Menor a mayor ASC (price)
  priceASC.addEventListener("click", () => {
    console.log("asc");
    fruts.sort((a, b) => a.price - b.price);
    renderFruts();
  });

  deleteLastFrut.addEventListener("click", () => {
    fruts.pop();
    renderFruts();
  });

  deleteFirstFrut.addEventListener("click", () => {
    fruts.shift();
    renderFruts();
  });

  addFrut.addEventListener("click", () => {
    const userFrut = prompt("¿qué fruta deseas agregar?");
    fruts.push(userFrut);
    renderFruts();
  });
});
